...
core_points=np.array([55,24]) # Position of Nucleus on Image
nameofknots='M87'
filename=nameofknots+'.fits'
#Downstream_Bounds=np.array([72.0,196.0])
Upstream_Bounds=np.array([55,10])
...
## Go colu.mn by column to calculate the max flux for each column
...
x=intensity_xpos
y=intensity_ypos
x_smooth=np.linspace(Upstream_Bounds[1],width,num=100)
y_smooth = spline(x, y, x_smooth)

## Define Axis
plt.contour(file1,30, cmap='gray')
plt.plot(x_smooth,y_smooth,c='y')
